...
def __init__(
        ...
        super(DDP, self).__init__(config=config, module=module)
        ...
        if int(os.getenv("USE_EPX", 0)):
            ...
            def _epx_load(state_dict):
                return self.module.load_state_dict(state_dict)

            logger.info(f"ddp start initialization epx.lcp.register_module")
            lcp = parallel_state.get_epx_data_parallel_lcp()
            lcp.register_module('ddp', _epx_dump, _epx_load)
            logger.info(f"ddp finish initialization epx.lcp.register_module")
...
for k in sys.modules:
    if k.startswith('megatron'):
        for target in ['DistributedDataParallel']:
            if getattr(sys.modules[k], target, None):
                setattr(sys.modules[k].DistributedDataParallel, "__init__", __init__)