...
#
# run without epx:
# ./run_deepseekv2_epx_fte.sh
#
# run with epx fte mode:
# ./run_deepseekv3_epx_fte.sh -u -r 0 --replica_parallel_size 2 # on master addr: 10.18.32.3
# ./run_deepseekv3_epx_fte.sh -u -r 1 --replica_parallel_size 2 --epx_master_addr 10.18.32.3
...
setup_epx_env() {
    ...
    export EPX_FTPG_ASSEMBLE_TIMEOUT_MS_DATA_PARALLEL_GROUP=60000
    export EPX_FTPG_ASSEMBLE_TIMEOUT_MS_DATA_PARALLEL_GROUP_WITH_CP=60000
...
set_common_env() {
    ...
    export PP_SIZE=2
    export EP_SIZE=2
    export WORLD_SIZE=4 # world size of a single replica
    export MICRO_BATCH_SIZE=1
    export NUM_MICROBATCHES=1

    export NUM_LAYERS=2
    export MOE_NUM_EXPERTS=4
    export MOE_ROUTER_NUM_GROUPS=2
    export MOE_ROUTER_GROUP_TOPK=1
    export MOE_ROUTER_TOPK=1
